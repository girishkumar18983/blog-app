...
function Profile() {
  ...
  const handleChangeUsername = async (e) => {
    ...
    try {
      ...
      // Update global context so the user stays logged in with their new name and token
      updateUser(data);
      setNewUsername('');
  ...
  return (
    <div className="page-container">
      <div className="hero" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
        ...
        <h1 style={{ margin: 0 }}>{profile?.name || user.username}</h1>
        {profile?.name && <p style={{ margin: 0, fontWeight: 'bold', color: 'var(--text-muted)' }}>@{user.username}</p>}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              {profile.friends.map(friend => (
                <div key={friend._id} className="post-card-author" style={{ padding: '0.8rem', background: 'var(--bg-input)', borderRadius: 'var(--radius-sm)' }}>
                  <div className="post-card-author-avatar">{friend.username[0].toUpperCase()}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            {searchResults.map(u => (
              <div key={u._id} className="post-card-author" style={{ padding: '0.8rem', background: 'var(--bg-input)', borderRadius: 'var(--radius-sm)', justifyContent: 'space-between' }}>
                ...
                <button onClick={() => addFriend(u._id)} className="btn btn-secondary" style={{ padding: '0.4rem 0.8rem' }}>
